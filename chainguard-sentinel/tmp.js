...
function fetchContractState(runtime2, contract) {
  try {
    runtime2.log(`Fetching state for contract: ${contract.name} (${contract.address})`);
    ...
    const nativeBalance = fetchNativeBalance(runtime2, evmClient, contract.address);
    stateData.nativeBalance = nativeBalance;
    runtime2.log(`Native balance: ${formatUnits(nativeBalance, 18)} ETH`);
    if (contract.abi && contract.monitoredFunctions && contract.monitoredFunctions.length > 0) {
      const functionResults = fetchCustomFunctions(runtime2, evmClient, contract.address, contract.abi, contract.monitoredFunctions);
    ...
    runtime2.log(`Error fetching contract state: ${msg}`);
...
function fetchCustomFunctions(runtime2, evmClient, contractAddress, abi, functionNames) {
  const results = [];
  for (const functionName of functionNames) {
    try {
      const abiFunction = abi.find((item) => item.type === "function" && item.name === functionName);
      if (!abiFunction) {
        runtime2.log(`Function ${functionName} not found in ABI`);
        continue;
      }
      ...
      runtime2.log(`Error calling function ${functionName}: ${err}`);
    }
  }
  return results;
}
...
function fetchPriceFeed(runtime2, feedConfig, chainSelectorName) {
  try {
    runtime2.log(`Fetching price feed: ${feedConfig.pairName}`);
    const network248 = getNetwork({
      chainFamily: "evm",
      chainSelectorName,
    ...
    runtime2.log(`${feedConfig.pairName}: $${priceFormatted.toFixed(2)} (updated ${lastUpdateAgo}s ago)${isStale ? " [STALE]" : ""}`);
    return feedData;
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    runtime2.log(`Error fetching price feed ${feedConfig.pairName}: ${msg}`);
    throw new Error(`Failed to fetch ${feedConfig.pairName}: ${msg}`);
  }
}
function fetchHistoricalPrices(runtime2, feedConfig, chainSelectorName, numRounds = 24) {
  try {
    runtime2.log(`Fetching ${numRounds} historical rounds for ${feedConfig.pairName}`);
    const network248 = getNetwork({
      chainFamily: "evm",
      chainSelectorName,
      isTestnet: chainSelectorName.includes("testnet")
    ...
    const evmClient = new ClientCapability(network248.chainSelector.selector);
    const latestData = fetchPriceFeed(runtime2, feedConfig, chainSelectorName);
    const latestRoundId = latestData.roundId;
    const prices = [];
    for (let i2 = 0;i2 < numRounds; i2++) {
      const roundId = latestRoundId - BigInt(i2);
      try {
        const callData = encodeFunctionData({
          abi: getChainlinkAggregatorV3Abi(),
        ...
        runtime2.log(`Round ${roundId} not found, skipping`);
      }
    }
    runtime2.log(`Fetched ${prices.length} historical prices for ${feedConfig.pairName}`);
    return prices;
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    runtime2.log(`Error fetching historical prices: ${msg}`);
    return [];
  }
}
function calculateVolatility(prices) {
  if (prices.length < 2)
    return 0;
  const returns = [];
  for (let i2 = 0;i2 < prices.length - 1; i2++) {
    const returnVal = (prices[i2] - prices[i2 + 1]) / prices[i2 + 1];
...
function checkDepeg(currentPrice, targetPrice = 1, tolerance = 0.02) {
...
function buildMarketDataSnapshot(runtime2, contract) {
  const snapshot = {
    timestamp: new Date(runtime2.now()).toISOString(),
    contractAddress: contract.address,
    chainSelectorName: contract.chainSelectorName
...
function formatMarketData(data) {
  ...
  if (data.customMetrics) {
    ...
    for (const [key, value2] of Object.entries(data.customMetrics)) {
      parts.push(`  - ${key}: ${value2}`);
    }
  }
...
function formatContractState(state) {
  ...
  if (state.nativeBalance !== undefined) {
    parts.push(`- Native Balance: ${Number(state.nativeBalance) / 1000000000000000000} ETH`);
  }
...
function evaluateRisk(runtime2, contract, marketData, contractState, aiAnalysis) {
  ...
  const overallRiskScore = Math.round(ruleBasedScore * 0.7 + aiRiskScore * 0.3);
  const shouldAlert = determineShouldAlert(violations, aiAnalysis.riskLevel, overallRiskScore);
  runtime2.log(`Risk Score: ${overallRiskScore}/100 | Violations: ${violations.length} | AI Risk: ${aiAnalysis.riskLevel} | Alert: ${shouldAlert}`);
  const assessment = {
    contractAddress: contract.address,
...
function checkThresholdViolations(thresholds, marketData) {
  const violations = [];
  if (thresholds.depegTolerance !== undefined && marketData.priceDeviationFromPeg !== undefined) {
    if (marketData.priceDeviationFromPeg > thresholds.depegTolerance) {
      violations.push({
        ...
        severity: calculateViolationSeverity(marketData.priceDeviationFromPeg, thresholds.depegTolerance, 2)
      });
    }
  }
  if (thresholds.volatilityMax !== undefined && marketData.volatility24h !== undefined) {
    if (marketData.volatility24h > thresholds.volatilityMax) {
      violations.push({
        ...
        severity: calculateViolationSeverity(marketData.volatility24h, thresholds.volatilityMax, 2)
      });
    }
  }
  if (thresholds.liquidityDropMax !== undefined && marketData.liquidityChange24h !== undefined) {
    ...
    if (liquidityDrop > thresholds.liquidityDropMax) {
      violations.push({
        ...
        severity: calculateViolationSeverity(liquidityDrop, thresholds.liquidityDropMax, 2)
      });
    }
  }
  if (thresholds.collateralRatioMin !== undefined && marketData.collateralRatio !== undefined) {
    if (marketData.collateralRatio < thresholds.collateralRatioMin) {
      ...
      violations.push({
        ...
        severity: calculateViolationSeverity(deviation, thresholds.collateralRatioMin * 0.1, 2)
      });
    }
  }
  if (thresholds.gasPriceMax !== undefined && marketData.currentGasPrice !== undefined) {
    if (marketData.currentGasPrice > thresholds.gasPriceMax) {
      violations.push({
        ...
        severity: calculateViolationSeverity(marketData.currentGasPrice, thresholds.gasPriceMax, 3)
      });
    }
  }
  return violations;
}
function calculateRuleBasedRiskScore(violations, marketData) {
...
function calculateViolationSeverity(currentValue, thresholdValue, criticalMultiplier = 2) {
...
function determineShouldAlert(violations, aiRiskLevel, overallRiskScore) {
  const hasCriticalViolation = violations.some((v) => v.severity === "CRITICAL" || v.severity === "HIGH");
